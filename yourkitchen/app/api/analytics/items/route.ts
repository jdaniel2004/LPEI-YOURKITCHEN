...
export async function GET(req: Request) {
  ...
  const { data, error } = await supabaseAdmin
    .from("order_lines")
    .select("name, qty, unit_price, extra_price, cancelled, item_id, created_at, ready_at, order:orders!inner(paid_at, status)")
  ...
  return Response.json({ standalone, prep, avgPrepMin });
}
